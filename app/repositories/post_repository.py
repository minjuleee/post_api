...
#       commit과 rollback은 트랜잭션의 컨트롩 타워인 [Service단]에서 담당한다.
#       Service단에서 트랜잭션을 컨트롤 하기 위해서, flush()는 해야 함!!!
# 
# ============================================================

from sqlalchemy.orm import Session, joinedload  
# 관계잇는 테이블을 미리 join으로 함께 가져올때 사용
...
class PostRepository :
  ...
  def create_post_tx(self, title:str, content:str, author:str) -> Post :
    ...
    self.db.add(post) # SqlAlchemy 세션에 post 객체 등록
    # insert sql이 db에 전달 되어 실행되어 id가 생성
  ...
  def get_with_relations(self, post_id:int) -> Optional[Post]:
    ...
    """
    return (
      self.db.query(Post)             # Post 객체에 대한 기본 select 쿼리문
      .options(
  ...
  def insert(self, title:str, content:str, author:str) -> Post :
    """
    게시글을 DB에 저장하고 Post객체(id, created_at, updated_at 포함)를 반환합니다.
    """
    print("게시글을 실제 딥에 저장하자! 요기는 레포지토리 단!!!")
    post = Post(
      title=title,
      content=content,
      author=author,
  ...
  def get_post_list(self, offset:int=0, limit:int=10,
      search:Optional[str]=None, author:Optional[str]=None,
      order_by:str="latest"
    ) -> List[Post]:
    """
    게시글 전체 목록 조회
    ...
    if order_by == "views" :  # 정렬 조건이 조회수 순일때
      ...
      query = query.order_by(Post.created_at.desc())   # 작성일 최신순(default)
      
      
    # "limit 옵셋, 페이지당 출력될 행의 수"를 붙여 실행하여 반환
    return query.offset(offset=offset).limit(limit=limit).all()  
  
  
  def get_post_count(self, search:Optional[str]=None, author:Optional[str]=None) -> int :
    """
    게시글의 전체 row수를 반환
    search, author가 있으면 그 값에 따라 조회된 row수 반환
    """
  ...
  def update(self, post:Post, changes:dict) -> Post :
    """ 
    변경할 필드 딕셔너리를 받아 게시글을 수정 
    post : 원래 글
    changes : 변경할 내용들
    """
    for field, value in changes.itmes() :
