...
from app.schemas.post_schema import (
  PostCreate, PostDetail, PostListResponse, 
  PostItem, PagingInfo, PostUpdate, PostCreateWithAttachment,
  PostDetailWithStat
)
...
class PostService:
  ...
  def create_post(self, data:PostCreate) -> PostDetail:
    ...
    # post 객체가 PostDetail(Pydantic 객체)에 유효한지 검증한 뒤 통과되면 PostDetail 객체 반환
  ...
  def get_post_detail(self, id:int) -> PostDetail :
    ...
    select + update 문이 하나의 db세션에 의해 처리되었다 => 트랜잭션 처리
    """
  ...
  def get_list(self, page:int, per_page:int, 
    ...
    count = self.repo.get_post_count(search, author)
    ...
    # Repository단에서 반환되는 List[Post]는 Post객체를 List[]에 감싼 타입이다. (json이 아님) 
    # => ORM에서 반환되는 기본값.
    # 그래서 우리는 PostItem(한건의 게시글 json) schema를 감싼 PostListResponse 
    # 스키마를 만들어 반환하는 구조를 선택
  ...
  def create_post_with_attachments(
    ...
    """
    try:
      post = self.repo.create_post_tx(data.title, data.content, data.author)  # post.id
  ...
  def get_post_with_stat(self, id:int) -> PostDetailWithStat :
    """ Post + PostStat + Attachment 조회 """
    post = self.repo.get_with_relations(id)
    if not post:
      raise HTTPException(
        status_code=500,
        detail=f"게시글을 가져오는 중 오류가 발생 했습니다",
      )
    return PostDetailWithStat.model_validate(post)
